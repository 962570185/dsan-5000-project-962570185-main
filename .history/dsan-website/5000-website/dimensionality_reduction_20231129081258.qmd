...
df.to_csv(../../data/others/IMDB_Top_250_Movies_ver2.csv)
```
...
name_vec  =  vectorizer.fit_transform(corpus)   
name_feature =np.array(name_vec.todense())
maxs=np.max(name_feature,axis=0)
name_feature=np.ceil(name_feature/maxs)


# 将得到的稀疏矩阵转换为DataFrame
df_name_vectors = pd.DataFrame(name_feature, columns=vectorizer.get_feature_names_out())
df = pd.concat([df, df_name_vectors], axis=1)


# 将新的文本向量DataFrame与原始DataFrame合并
df = df.drop(["name"], axis=1).join(name_feature)
