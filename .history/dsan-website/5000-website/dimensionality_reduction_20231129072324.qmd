...
# 'Genres' 列转换为列表格式，适用于 MultiLabelBinarizer
df['Genres'] = df['Genres'].apply(lambda x: x.split(','))

# 使用 MultiLabelBinarizer 进行独热编码
df_genres = mlb.fit_transform(df['Genres'])

# 将得到的矩阵转换为 DataFrame，并添加适当的列名
df_genres = pd.DataFrame(df_genres, columns=mlb.classes_)

# 将新的独热编码的DataFrame与原始DataFrame合并
df = df.drop('Genres', axis=1).join(df_genres)
